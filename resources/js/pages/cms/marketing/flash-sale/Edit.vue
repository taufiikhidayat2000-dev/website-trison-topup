...
const editOriginalPrice = ref<number | null>(null);
...
const openEditRow = (row: any) => {
    ...
    editOriginalPrice.value = row.original_price;
    editFlashPrice.value = row.flash_price;
    editFlashStock.value = row.flash_stock;
    editStatus.value = row.status;
};

// Live preview of the computed Flash Price for percent-based discounts, so
// the admin can see the effect of marking up "Harga Sebelum" before saving
// (Flash Price = Harga Sebelum x (1 - diskon%), falling back to the
// product's real sell_price when Harga Sebelum is left empty).
const editFlashPricePreview = computed(() => {
    if (
        editPricingType.value !== 'percent' ||
        !editingRow.value ||
        editDiscountPercent.value === null
    ) {
        // Diskon wajib diisi untuk mode persen (lihat validasi backend) - jangan
        // tampilkan preview seolah 0% valid sebelum admin benar-benar mengisinya.
        return null;
    }

    const base = editOriginalPrice.value || editingRow.value.product?.sell_price || 0;
    const percent = editDiscountPercent.value;
...
const submitEditRow = () => {
    ...
    router.patch(
        `/cms/marketing/flash-sales/${props.flashSale.id}/products/${editingRow.value.id}`,
        {
            ...
            original_price: editOriginalPrice.value,
    ...
    <Modal v-slot="{ close }">
        <div class="p-6">
            ...
            >
                ...
                <div class="grid gap-2">
                    ...
                    <Select
                        ...
                        :default-value="flashSale.countdown_style"
                    >
                        ...
                        <SelectContent>
                            <SelectItem value="detailed"
                                >Hari Jam Menit Detik</SelectItem
                            >
                ...
                <div class="grid gap-2">
                    <Label for="after_end_action"
                        >Setelah Flash Sale Berakhir</Label
                    >
                    <Select
                        name="after_end_action"
                        :default-value="flashSale.after_end_action"
                    >
                        <SelectTrigger id="after_end_action" class="w-full">
                            <SelectValue placeholder="Pilih aksi" />
                        </SelectTrigger>
                        <SelectContent>
                            <SelectItem value="revert_price"
                                >Kembali ke Harga Normal</SelectItem
                            >
                            <SelectItem value="hide"
                                >Sembunyikan Flash Sale</SelectItem
                            >
                            <SelectItem value="sold_out"
                                >Tampilkan Sold Out</SelectItem
                            >
                            <SelectItem value="keep_showing"
                                >Tetap Tampil</SelectItem
                            >
                        </SelectContent>
                    </Select>
            ...
            <div v-else class="mt-6 space-y-4">
                ...
                >
                    <div
                        class="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-lg border bg-background p-6 shadow-lg"
                    >
                        ...
                        <div class="space-y-4">
                            ...
                            >
                                ...
                                <div
                                    ...
                                    :class="{
                                        'opacity-50':
                                            product.already_in_conflicting_sale,
                                    }"
                                >
                                    <Checkbox
                                        :model-value="
                                ...
                                <Select v-model="pricingType">
                                    ...
                                    <SelectContent>
                                        <SelectItem value="percent"
                                            >Diskon Persen</SelectItem
                                        >
                                        <SelectItem value="manual"
                                            >Harga Manual</SelectItem
                                        >
                                    </SelectContent>
                                </Select>
                            </div>
                            ...
                            <div class="grid gap-2">
                                <Label>Harga Sebelum (Opsional)</Label>
                                <MoneyInput v-model="originalPrice" />
                                <p class="text-xs text-muted-foreground">
                                    Kosongkan untuk memakai harga jual produk
                                    saat ini. Markup harga ini dulu sebelum
                                    dipotong diskon untuk menampilkan harga
                                    coret yang berbeda dari harga asli.
                                </p>
                                <p
                                    v-if="
                                        originalPrice &&
                                ...
                                >
                                    Harga Sebelum ini akan dipakai sama untuk
                                    semua {{ selectedProductIds.length }}
                                    produk terpilih walau harga jual aslinya
                                    berbeda-beda. Tambah satu per satu kalau
                                    mau markup berbeda per produk.
                                </p>
                            </div>

                            <div
                                v-if="pricingType === 'percent'"
                                class="grid gap-2"
                            >
                                <Label>Diskon (%)</Label>
                                <Input
                ...
                <Table>
                    ...
                    <TableBody>
                        <TableEmpty
                            v-if="(flashSale.products ?? []).length === 0"
                            :colspan="7"
                        >
                            Belum ada produk di Flash Sale ini.
                        </TableEmpty>
                        <TableRow
                            v-for="row in flashSale.products ?? []"
                            :key="row.id"
                        >
                            <TableCell>
                            ...
                            <TableCell>
                                <span v-if="row.discount_percent"
                                    >{{ row.discount_percent }}%</span
                                >
                                <span v-else>-</span>
                            </TableCell>
                            <TableCell>
                ...
                >
                    ...
                    >
                        ...
                        <div class="space-y-4">
                            ...
                            <div class="grid gap-2">
                                <Label>Harga Sebelum (Opsional)</Label>
                                <MoneyInput v-model="editOriginalPrice" />
                                <p class="text-xs text-muted-foreground">
                                    Kosongkan untuk memakai harga jual produk
                                    saat ini ({{
                                        formatCurrency(
                                            editingRow?.product?.sell_price ??
                                                0,
                                        )
                                    }}). Markup harga ini dulu sebelum
                                    dipotong diskon untuk menampilkan harga
                                    coret yang berbeda dari harga asli.
                                </p>
                            </div>
                            ...
                            >
                                ...
                                />
                                <p
                                    v-if="editFlashPricePreview !== null"
                                    class="text-xs text-muted-foreground"
                                >
                                    Harga Flash Sale:
